...
router.post('/categorias/nova', eAdmin, (req, res) => {
	...
	if (erros.length > 0) {
		...
		new Categoria(novaCategoria).save().then(() => {
			...
			req.flash("error_msg", 'Categoria não pode ser criada, tente novamente!')
			res.redirect('/admin')
		})
	}
})
...
router.post('/categorias/edit', eAdmin, (req, res) => {
	Categoria.findOne({_id: req.body.id}).then((categoria) => {
		...
		categoria.save().then(() => {
			...
			req.flash('error_msg', 'Houve um erro interno ao editar a categoria = ' + err)
			res.redirect('/admin/categorias')
		})
	}).catch((err) => {
...
router.post('/postagens/nova', eAdmin, (req, res) => {
	...
	if (erros.length > 0) {
		...
		new Postagem(novaPostagem).save().then(() => {
			...
			req.flash('error_msg', 'Houve um erro com o salvamento da Postagem!')
			res.redirect('/admin/postagens')
...
router.get('/postagens/edit/:id', eAdmin, (req, res) => {
	Postagem.findOne({ _id: req.params.id }).lean().then((postagem) => {
		Categoria.find().lean().then((categorias) => {
			res.render('admin/editarpostagem', { categorias: categorias, postagem: postagem })
		})
	}).catch((err) => {
		req.flash('error_msg', 'Esta Formulário não existe = ' + err)
		res.redirect('/admin/postagens')
	})
})
...
router.post('/postagens/edit', eAdmin, (req, res) => {
	Postagem.findOne({ _id: req.body.id }).then((postagem) => {
		...
		postagem.save().then(() => {
			...
			req.flash('error_msg', 'Houve um erro interno ao editar a postagem = ' + err)
			res.redirect('/admin/postagens')
		})
	}).catch((err) => {
...
router.post('/postagens/deletar', eAdmin, (req, res) => {
	Postagem.deleteOne({ _id: req.body.id }).then(() => {
		req.flash('succes_msg', 'Postagem deletada com sucesso!')
		res.redirect('/admin/postagens')
	}).catch((err) => {
